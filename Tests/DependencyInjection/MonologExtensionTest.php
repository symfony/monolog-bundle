...
class MonologExtensionTest extends DependencyInjectionTest
{
    ...
    public function testSentryBreadcrumbHandlerWhenConfigurationIsOk(array $config, string $hubId)
    {
        ...
        $logger = $container->getDefinition('monolog.logger');
        $this->assertDICDefinitionMethodCallAt(0, $logger, 'useMicrosecondTimestamps', ['%monolog.use_microseconds%']);
        $this->assertDICDefinitionMethodCallAt(1, $logger, 'pushHandler', [new Reference('monolog.handler.sentry_breadcrumb')]);
        $this->assertDICDefinitionMethodCallAt(2, $logger, 'pushHandler', [new Reference('monolog.handler.sentry')]);
